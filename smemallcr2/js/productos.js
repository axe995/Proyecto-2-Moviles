...
function LoadCatalogoProductos(){
	...
	contenido += "<div class='col-xs-6'><a class='thumbnail btn-success animated flipInY' onclick='LoadFormEditarProductos()'><img src='../img/nuevoProducto.png' /></a></div>";
	var xmlhttp;
	...
	if (xmlhttp.readyState==4 && xmlhttp.status==200)
	{
	  ...
	  for (var i = 0; i < mainJson.RETURNVALUE.length; i++) {
	    ...
	    contenido += "<div class='col-xs-6'><a class='thumbnail btn-success animated flipInY'onclick='LoadFormEditarProductos2(\""+ productoJson.mKeyValue +"\")'><img src='"+ "../img/Producto.png" +"' /></a></div>";
	  }
...
function LoadFormEditarProductos(){
  ...
  $( "#dinamicContent" ).load("/HTML/formEditarProducto.html", function (response, status, xhr) {
        if (status == "success"){
